...
@section('content')
    <x-page-header title="New category" subtitle="Create a category for products.">
        <x-button href="/admin/categories" variant="secondary" size="sm">Back</x-button>
    </x-page-header>

    <x-card>
        <form method="POST" action="/admin/categories" class="grid gap-4">
            @csrf
            ...
            <div class="grid gap-1.5">
                ...
                <select id="parent_id" name="parent_id" class="w-full rounded-xl border border-slate-300 bg-white px-3.5 py-2.5 text-sm text-slate-900 focus:border-slate-400 focus:ring-2 focus:ring-slate-200">
                    ...
                    @foreach($categories as $c)
                        <option value="{{ $c->id }}" {{ (string)old('parent_id')===(string)$c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                    @endforeach
                </select>
